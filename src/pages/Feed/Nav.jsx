        ...
        {!user ? (
          ...
          <div className="flex flex-col gap-5">
            <div className="flex flex-col items-center gap-2">
              <img className="rounded-full h-[35px] md:h-[45px]" src={user.photoURL} alt="" />
              <p className="max-md:hidden">{user.displayName}</p>
            </div>
            <button onClick={()=>signOut(auth)} className="flex justify-center gap-2 p-1 items-center bg-zinc-700 rounded text-2xl md:text-[15px] transition hover:bg-zinc-900">
              <BiSolidDoorOpen />
